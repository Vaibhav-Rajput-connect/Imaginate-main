...
const Result = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    if (input) {
      try {
        const { data } = await axios.post(backendUrl + '/api/image/generate-image', { prompt: input }, { headers: { Authorization: `Bearer ${token}` } })
    ...
    <motion.form
      ...
      {!isImageLoaded &&
        <div className='flex w-full max-w-xl bg-neutral-500 text-white text-sm p-0.5 mt-10 rounded-full'>
          ...
          <button type='submit' className='bg-zinc-800 px-10 sm:px-16 py-3 rounded-full'>Generate</button>
        </div>
      }
      {isImageLoaded &&
